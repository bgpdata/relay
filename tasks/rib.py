...
def rib_task(queue, db, logger, events, memory):
    ...
    try:
        ...
        if len(buckets) > 0:
            ...
            for peer_index in peer_indexes:
                bgp_open = bytes(
                    ...
                    BGPOpen(
                        version=4,
                        my_as=23456 if peers[peer_index]["asn"] > 65535 else peers[peer_index]["asn"],
                        hold_time=180,
                        bgp_id=peers[peer_index]["bgp_id"],
                        opt_params=[
                            BGPOptParam(
                                param_type=2, param_value=[
                                    BGPCapMultiprotocol(afi=1 if peers[peer_index]["ip_address_family"] == "IPv4" else 2, safi=1),
                                    BGPCapGeneric(code=2),
                                    BGPCapFourBytesASN(asn=peers[peer_index]["asn"]) if peers[peer_index]["asn"] > 65535 else None
                                ]
                            )
                        ]
                    )
            ...
            for peer_index in peer_indexes:
                bgp_update = bytes(BGPHeader() / BGPUpdate())
                bmp_message = BMPv3.monitoring_message(
                    peers[peer_index]['ip_address'],
                    peers[peer_index]['asn'],
        ...
        logger.info(f"Queueing {len(messages)} BMP messages.")
