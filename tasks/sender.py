...
def sender_task(queue, db, logger, events, memory):
    ...
    try:
        ...
        logger.info(f"Connecting to {Config.COLLECTOR_CONNECT}")
        with socket.create_connection((Config.COLLECTOR_CONNECT.split(':')[0], int(Config.COLLECTOR_CONNECT.split(':')[1])), timeout=300) as sock:
            while True:
                try:
